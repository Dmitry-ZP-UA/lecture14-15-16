    ...
    @if ($parentcomment->parent_id === null)
        <div class="alert alert-info">
            ...
            <form action=" {{ route('comment.update', $product->slug) }}" method="post">
                ...
                <button type="submit" class="btn btn-primary">Like {{ $parentcomment->likes_counter }}</button>
            </form>
        ...
        @if ($commentchild->parent_id === $parentcomment->id)
            <div class="alert alert-info" style="margin-left: 50px;">
                ...
                <form action=" {{ route('comment.update', $product->slug) }}" method="post">
                    ...
                    <button type="submit" class="btn btn-primary">Like {{ $commentchild->likes_counter }}</button>
                </form>
            </div>
        @endif
    @endforeach
